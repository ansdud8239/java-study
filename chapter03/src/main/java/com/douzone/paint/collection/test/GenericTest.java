...
public class GenericTest {
	...
	public static void main(String[] args) {
		...
		MyStack<Drawable> m = new MyStack<>(20);
		
		drawShapeLayer(list1, null);
		drawShapeLayer(list2, m);
	...
	public static <T extends Shape> void draqShape(T t) {
		t.draw();
	}
	 ...
	 * 2) 부모듣만	(자식포함)
	 * 3) <T super Type> 오류
	 * 	- 함수 구현 블록(클래스 구현 블록) 안에서 T타입이 모호하기 때문에 구현이 불가능
	 * 	- Type Erasing의 결과 <T super Type>는 Object이기 때문에 문법적인 의미가 없다
	 * 	- 결론은 존재 하지 않는다
	 */
	// shape < drawable < paint < object
	//<T super Shape> -> <object>이므로 해당 코드는 사용하지 않음
//	public static <T super Shape> void draqShape(T t) {
//		t.draw();
//	}
